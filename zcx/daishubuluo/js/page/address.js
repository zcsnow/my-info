$(function(){
	...
	$(".address-submit-btn").click(function(){
		...
		if ($.trim($("#detail_addr").val()) == ""){
			...
			$("#detail_addr").addClass("error"),
        	!1;
		}else{
			$("#detail_addr").removeClass("error");
		}
	...
	$('.js-delete-ads-btn').click(function(e){
	 e.preventDefault();
	 var $this = $(this);
	 jShare("是否删除该地址？","",function(result){
			if(result===true){
				$this.parents('.my-address-list').remove();
				// if($('.my-address-list').length<=0){
	...
	$('.my-address-box').on('click','.set-ads-btn',function(e){
		...
		e.preventDefault();
		e.stopPropagation();
		$(this).addClass('cur').parents('.my-address-list').siblings().find('.cur').removeClass("cur");
