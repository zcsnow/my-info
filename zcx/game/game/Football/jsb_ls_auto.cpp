...
JSBool js_ls_Leafsoar_share(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSBool ok = JS_TRUE;
...
JSBool js_ls_Leafsoar_openUrl(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSBool ok = JS_TRUE;
...
void js_register_ls_Leafsoar(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
		...
		JS_FN("share", js_ls_Leafsoar_share, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FN("openUrl", js_ls_Leafsoar_openUrl, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FS_END
	};
