...
$.ajax({
	  type: "GET",
	  async:true,
	  url: baseUrl+"Member.aspx?op=MemberGetRcoin&user_id="+userId+"&pageindex="+ pageIndex +"&pagesize="+listNum,
	  ...
	  success: function (data) {
		  ...
		  $('.integral-num').html(data.R_coin||0);
...
//赚R币
$('.js-R-btn').on('click',function(){
	var RNum= $('.integral-num').html();
	  if(isAndroid){
		   //alert("Android");
	  	   test.goR(RNum);
	  }else{
		  //alert("ios");
		  window.webkit.messageHandlers.goR.postMessage(RNum);
	  }
});

//帮助按钮
$('.js-R-help').on('click',function(e){
	  
	  if(isAndroid){
		   //alert("Android");
	  	   window.location.href="Rcoin_help.html";
	  }else{
		  //alert("ios");
		  window.webkit.messageHandlers.RHelp.postMessage(null);
	  }
});
