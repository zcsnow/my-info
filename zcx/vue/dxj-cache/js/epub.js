...
EPUBJS.Render.Iframe.prototype.addIframe = function(){
	...
	if (this._width || this._height) {
		...
		var isIos = /\(i[^;]+;( U;)? CPU.+Mac OS X/i.test(window.navigator.userAgent);
		if(isIos){
			this.iframe.width = (window.innerWidth-23) + "px";
		}else{
			this.iframe.width = this._width;
		}
		//alert(this.iframe.width);
	}
	return this.iframe;
};
	...
	_store.getText(full).then(function(text){
		var url;
		 ...
		 EPUBJS.replace.cssImports(_store, full, text).then(function (importText) {
		  ...
		  importText = importText + "img.fullscreen{width:100%;display:block;}";
