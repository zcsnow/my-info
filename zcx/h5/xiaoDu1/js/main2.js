$(function(){
	...
	function makeImg(text){
		...
		img.onload = function(){
			...
			//请求图片合成
			var uploadUrl = convertCanvasToImage(newCanvas, true);
			var url = "http://localhost:3000/api/uploadImage";
			var res_reslut = $.ajax({
				type:"post", 
				url: url, 
				async: true ,
				...
				success: function (data) {
						console.log(data);
						var imgSrc = 'CreateImage/'+ data.message.imgName + '.jpeg';
						$('.page-last .bg').find('img').attr('src',imgSrc);
						setTimeout(function(){
						  $('.pop-tip').hide();
						  $('.page-name').hide();
						  $('.page-last').show();
