...
(function() {
	...
	var n = {
		...
		i = {
			...
			buttonLayoutEventHandler: function(e, t, r) {
				...
				if (s = i[u]) if (m.comm.collision(s.x, s.y, s.width, s.height, t - 5, r - 5, 10, 10)) {
					switch (e) {
					case "mousedown":
					case "touchstart":
	...
	link = {
		...
		objExtend: function() {
			...
			for (; t < n; t++)
			if ((i = arguments[t]) != null) for (var s in i) {
				var o = e[s],
					u = i[s];
				if (e === u) continue;
...
define("host1", ["lib/link"], function(e) {
    ...
    return {
        get: function(s, o) {
			console.log(new e.action.role([], 0, 0, 0, o ? o : t, n, r, u || i));
			console.log(0);
            var u = s >= 0 && i[s] ? [i[s]] : i;
            return new e.action.role([], 0, 0, 0, o ? o : t, n, r, u || i)
        }
...
define("index", ["lib/link", "lib/action", "shine"], function(e, t, r) {
    var n1,n2;
	return {
		init: function() {
			  /*c(2);*/
    ...
    return {
        get: function(s, o) {
			...
			e.run(function() {
				var t = Date.now();
				e.canvas.fillStyle("#FFF").fillScreen().drawImage("bg", i - 800 >> 1, u - 220);
				...
				var c = a(l.time);
				//console.log(c);
				e.canvas.fillStyle("#000").font("24px Arial").fillText(c, i - e.canvas.measureText(c).width - 20, 45), t = null
			}).menu(function() {
				e.canvas.fillStyle().fillScreen()
				.drawImage("bg", i - 800 >> 1, u - 220)
                ...
                if(e.buttonLayout.released("rank")) {
                    dp_Ranking()
                }
				if(e.buttonLayout.released("help")) {
                    help_text();
                }
				...
				.fillStyle("#000")
				.drawString("成绩斐然，工行融易购手机流量电子卷为你加油", 0, u + 460, e.graphics.VCENTER, !1, null, null, "24px 微软雅黑"), 
			    //.fillStyle("#000").drawString("马上抽取", 0, u + 520, e.graphics.VCENTER, !1, null , null , "30px 微软雅黑")
				l.time > l.bestTime && e.canvas.fillStyle("#FF0").drawString("新纪录", 0, u + 240, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑"), 
				e.buttonLayout.released("lottery-btn") && lottery_btn(), 
				e.buttonLayout.released("return") ? f() : e.buttonLayout.released("restart") && dp_share2(l.time)*/
				var curTime = (l.time/1000).toFixed(2);
				//var resultTxt = "说好的一万年呢";
				var scorePrecent = 0;
				var baseNum = 0;
				if(curTime<30){
					//resultTxt = "说好的一万年呢";
					$('.result-title').find('img').attr('src','images/p3_img1.png');
					if(curTime<5){
						baseNum=10;
						scorePrecent = (baseNum + parseInt(Math.random()*baseNum));
						$('.score-txt em').text(scorePrecent);
						...
						baseNum=40;
						scorePrecent = (baseNum + parseInt(Math.random()*20));
						$('.score-txt em').text(scorePrecent);
					...
					if(curTime>=30&&curTime<40){
						baseNum=80
						scorePrecent = (baseNum + parseInt(Math.random()*5));
						$('.score-txt em').text(scorePrecent);
					}else if(curTime>=40&&curTime<60){
						baseNum=85;
						scorePrecent = (baseNum + parseInt(Math.random()*5));
						$('.score-txt em').text(scorePrecent);
					}else if(curTime>=60&&curTime<80){
						baseNum=90;
						scorePrecent = (baseNum + parseInt(Math.random()*5));
						$('.score-txt em').text(scorePrecent);
					}else{
						baseNum=95;
						scorePrecent = (baseNum + parseInt(Math.random()*4));
						$('.score-txt em').text(scorePrecent);
					}
					//resultTxt = "坚持这么久，一定是真爱";
					$('.result-title').find('img').attr('src','images/p3_img2.png');
					$('.score-txt em').text();
					//console.log(resultTxt);
				}
	
				var strs= new Array(); 
				strs=String(curTime).split(""); 
				if(strs.length==4){
					$('.score .time-num1').text(0);
					$('.score .time-num2').text(strs[0]);
					$('.score .time-num3').text(strs[1]);
					$('.score .time-num4').text(strs[2]);
					$('.score .time-num5').text(strs[3]);
					
				}
				if(strs.length==5){
					$('.score .time-num1').text(strs[0]);
					$('.score .time-num2').text(strs[1]);
					$('.score .time-num3').text(strs[2]);
					$('.score .time-num4').text(strs[3]);
					$('.score .time-num5').text(strs[4]);
				...
				$('.canvas-box .cloud1').removeClass('cloudMove1');
	            $('.canvas-box .cloud2').removeClass('cloudMove2');
				$('.canvas-box .fly1').removeClass('flyMove1');
	            $('.canvas-box .fly2').removeClass('flyMove2');
				$('.result-box .dog-icon').addClass('dogMove');
	            $('.result-box .dog-p').addClass('dog-p-move');
				$('.canvas-box').hide();
				$('.result-box').show();
				
				$('.restart').on('click',function() {
					window.location="index.html?"+ new Date().getTime()+"#1";
					/*$('.result-box').hide();
					$('.canvas-box').show();
			...
			var f = function() {
					//e.buttonLayout.clear().create().base().gameFlow.menu()
				};
			f();
			var l = {
				moduleName: "",
