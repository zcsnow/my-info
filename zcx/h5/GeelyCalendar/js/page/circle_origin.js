...
function encourage(perfection) {
    if (perfection <= 0.5) {
        ...
        return '亲，你的几何是体育老师教的么？你画的圆圆度才' + (perfection * 100).toFixed(2) + '%，有点像90年代的中国企业较以前发展壮大了，但品牌还不成气候。';
    } else if (perfection < 0.9 && perfection>=0.7) {
        return '不用说，你的几何肯定是数学老师教的咯。你画的圆圆度达到' + (perfection * 100).toFixed(2) + '%，是2000年后的中国品牌在国内外开始产生一定的影响力';
    } else if (perfection >= 0.9) {
        return '行走的圆规！你画的圆圆度达到' + (perfection * 100).toFixed(2) + '%,就像2016年吉利品牌成立30年，高质感中级车吉利帝豪GL即将以自胜者脚步迈向世界舞台，中国的月亮才更圆。';
    }
